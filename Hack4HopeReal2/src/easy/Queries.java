...
public static List<User> printUsersByPoints() 
{
	
connectToDB();
try {
Statement stmt = conn.createStatement();
    ResultSet rs = stmt.executeQuery("SELECT * FROM `eogotwa_h4hProject`.`User` ORDER BY points DESC");
    ulist.clear();
    while(rs.next()){
   
    User u = new User(rs.getString("username"),rs.getString("fName"),rs.getString("lName"),rs.getInt("points"), rs.getString("color"));
    ulist.add(u);
...
try {
Statement stmt = conn.createStatement();
    ResultSet rs = stmt.executeQuery("SELECT * FROM `eogotwa_h4hProject`.`User` WHERE username = '" + username+"'");
    while(rs.next()){
    //	System.out.println("here");
    use = new User(rs.getString("username"),rs.getString("fName"),rs.getString("lName"),rs.getInt("points"), rs.getString("bio"), rs.getString("color"));
    //System.out.println(rs.getString("username"));
    }
...
public static int getNumPoints(String un) {
	int points = 0;
	connectToDB();
	try {
	Statement stmt = conn.createStatement();
	ResultSet rs = stmt.executeQuery("SELECT `points` FROM `eogotwa_h4hProject`.`User` WHERE username = " + un);
	while(rs.next()){
...
public static String getTitlebyID(int pID) {
	String title = "";
	connectToDB();
	try {
	Statement stmt = conn.createStatement();
	ResultSet rs = stmt.executeQuery("SELECT title FROM `eogotwa_h4hProject`.`Post` WHERE postID=" + pID );
	while(rs.next()){
